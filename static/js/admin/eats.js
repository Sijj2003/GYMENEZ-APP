...
async function fetchAllAdminEatsCoreData() {
    try {
        const [resFoods, resUsers] = await Promise.all([
            fetch(`${API_BASE_URL}/api/admin/foods`, { headers: getSecureHeaders() }),
            fetch(`${API_BASE_URL}/api/admin/users`, { headers: getSecureHeaders() })
        ]);
...
function triggerAdminFoodFilter() {
    ...
    foodSearchTimeout = setTimeout(() => {
        const term = document.getElementById('admin-food-search').value.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
        ...
        const filtered = allFoodsData.filter(f => {
            ...
            const fCat = (f.category||'').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
            return fName.includes(term) || fCat.includes(term);
        });
...
// ==========================================
// 🎴 MODAL CRUD ALIMENTOS (CON TRADUCTOR INTELIGENTE)
// ==========================================
function openFoodModal(food = null) {
    ...
    if (!food) {
        ...
        // 🧠 TRADUCTOR INMUNE A ERRORES DE CATEGORÍA
        const selectCat = document.getElementById('form-food-category');
        const dbCat = food.category || '';
        ...
        if (exactMatch) {
            ...
            // Comprobación por palabras clave inmune a mayúsculas/minúsculas y tildes
            const catLower = dbCat.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
            
            if (catLower.includes('proteina') && catLower.includes('grasa')) selectCat.value = 'Proteínas Grasas AVB';
            else if (catLower.includes('proteina')) selectCat.value = 'Proteínas Magras AVB';
            else if (catLower.includes('carbohidrato') && (catLower.includes('rapido') || catLower.includes('fruta'))) selectCat.value = 'Carbohidratos Rápidos y Frutas';
            else if (catLower.includes('carbohidrato') || catLower.includes('almidon')) selectCat.value = 'Carbohidratos Almidonados';
            else if (catLower.includes('leguminosa')) selectCat.value = 'Leguminosas';
            else if (catLower.includes('lipido') || catLower.includes('grasa')) selectCat.value = 'Lípidos y Grasas Esenciales';
            ...
            else if (catLower.includes('suplement')) selectCat.value = 'Suplementación y Ergogénicos';
            else selectCat.value = 'Otro';
        }

        // Llenar macros
        document.getElementById('form-food-calories').value = food.calories || 0;
        document.getElementById('form-food-proteins').value = food.proteins || 0;
        document.getElementById('form-food-carbs').value = food.carbs_net || 0;
        
        const grasaTotal = Number(food.fats_saturated || 0) + Number(food.fats_unsaturated || 0);
        document.getElementById('form-food-fats').value = grasaTotal.toFixed(2);
        
        // Switches Inmunológicos (Verifica que existan en el DOM antes de asignarlos)
        if(document.getElementById('form-food-gluten')) document.getElementById('form-food-gluten').checked = food.has_gluten === true;
        if(document.getElementById('form-food-lactose')) document.getElementById('form-food-lactose').checked = food.has_lactose === true;
        if(document.getElementById('form-food-nuts')) document.getElementById('form-food-nuts').checked = food.has_nuts === true;
        if(document.getElementById('form-food-sodium')) document.getElementById('form-food-sodium').checked = food.high_sodium === true;
    }
...
async function handleFoodFormSubmit(e) {
    ...
    const payload = {
        ...
        fats_unsaturated: 0,
        has_gluten: document.getElementById('form-food-gluten') ? document.getElementById('form-food-gluten').checked : false,
        has_lactose: document.getElementById('form-food-lactose') ? document.getElementById('form-food-lactose').checked : false,
        has_nuts: document.getElementById('form-food-nuts') ? document.getElementById('form-food-nuts').checked : false,
        high_sodium: document.getElementById('form-food-sodium') ? document.getElementById('form-food-sodium').checked : false
    };
...
async function openDietModal(userId, fullName) {
    ...
    try {
        const res = await fetch(`${API_BASE_URL}/api/admin/diet/${userId}`, { headers: getSecureHeaders() });
        const data = await res.json();
...
async function executeDeleteDiet() {
    ...
    try {
        const res = await fetch(`${API_BASE_URL}/api/admin/diet/${userId}`, { method: 'DELETE', headers: getSecureHeaders() });
        const data = await res.json();
