...
// Inyección de la UI Flotante
const toastHTML = `
<div id="radar-toast" class="fixed top-6 right-6 z-[9999] transform translate-x-[150%] transition-transform duration-500 flex items-center gap-3 bg-[#1c1c1e]/95 backdrop-blur-xl border border-white/10 p-3 pr-6 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.8)] cursor-pointer hover:bg-[#2c2c2e] hover:scale-105" onclick="window.location.href='/apps/user/pulse.html'">
        ...
        if (res.ok && data.success) {
            ...
            onSnapshot(doc(db, "chats", userId), (docSnap) => {
                ...
                if (docSnap.exists() && docSnap.data().unread_user) {
                    ...
                    toast.classList.remove('translate-x-0'); toast.classList.add('translate-x-[150%]');
                }
            });
        }
    } catch(e) {}
